...
public class PickerManager: NSObject {
    ...
    class public var pickerPhotoSize: CGSize {
        assert(ImagePickerOptions.photoLineNum > 2, "列数最小为2")
        ...
        let width = (screenWidth - CGFloat(ImagePickerOptions.photoInterval) * (CGFloat(ImagePickerOptions.photoLineNum) - 1)) / CGFloat(ImagePickerOptions.photoLineNum)
        ...
        if photoAsset != nil {
            ...
            self.photoManage.requestImage(for: photoAsset!, targetSize: photoScaleSize, contentMode: .aspectFill, options: self.photoOption, resultHandler: { (image, infoDic) in
                if image != nil {
                    ...
                    let image_buffer = ImagePickerBundle.imageFromBundle("image-buffer")
    ...
    public func getPhotoDefalutSize(_ ablumIndex: Int, photoIndex: Int, resultHandler: ((UIImage?, [AnyHashable: Any]?) -> Void)?) {
        getPhoto(ablumIndex, photoIndex: photoIndex, photoSize: PickerManager.pickerPhotoSize, resultHandler: resultHandler)
    }
    ...
    private func getPhotoAlbum() {
        ...
        //将获取的相册加入到相册的数组中
        photoAlbums.append([ImagePickerBundle.localizedString(key: "全部照片"): allResult])
